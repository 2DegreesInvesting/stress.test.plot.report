...
analysis_data_1_run <- analysis_data_full_global |> 
  ...
  dplyr::inner_join(analysis_data %>% distinct(company_id, ald_sector, ald_business_unit))

data_plot <- analysis_data_1_run |> 
  
  dplyr::group_by(term, ald_sector, ald_business_unit) |>
  dplyr::summarise(avg_pd_baseline=mean(pd_baseline),
                   avg_pd_shock=mean(pd_shock)
                   , .groups="drop") |>
...
le_plot <- ggplot(data_plot, aes(x=term, y=`Average PD`, group=`PD type`, color=`PD type`)) +
  geom_line() +
  ...
  facet_wrap(~ald_sector+ald_business_unit, scales = "fixed") +
  theme(panel.grid.major.y = element_line(color = "gray", linetype = "dashed", linewidth=0.3)) +
  ggtitle(paste(analysis_data_1_run[1, "shock_scenario"] %>% pull(),
              " - shock year :",
...
# 5776a4b8-476f-48ba-91ae-b83a178d4331 // filter(shock_scenario=="NGFS2021_GCAM_NZ2050", shock_year==2032)
# 24e5ddb1-d63b-416f-bc4f-6d5f2d0e0a5f // filter(shock_scenario=="Oxford2021_fast", shock_year==2032)
# 663fa2de-e808-457b-b234-dca8e736b8a0 // filter(shock_scenario=="NGFS2021_REMIND_B2DS", shock_year==2032) 
# eb594eb8-13a7-461e-b8fb-7cad52a7167b // filter(shock_scenario=="NGFS2021_GCAM_NZ2050", shock_year==2027)

for (run_id in c("5776a4b8-476f-48ba-91ae-b83a178d4331", "24e5ddb1-d63b-416f-bc4f-6d5f2d0e0a5f", 
                 "663fa2de-e808-457b-b234-dca8e736b8a0", "eb594eb8-13a7-461e-b8fb-7cad52a7167b")){
analysis_data_1_run <- analysis_data_full_global_term_5 |> dplyr::filter(run_id == run_id)
...
title <- cowplot::ggdraw() + 
  cowplot::draw_label(
    paste(analysis_data_1_run[1, "shock_scenario"] %>% pull(),
              " - shock year :",
              analysis_data_1_run[1, "shock_year"] %>% pull()),
    fontface = 'bold',
    x = 0.5,
...
remove_outliers <- function(df, column, max_zscore=3) {
  ...
  outlier_companies <- unique(df[abs(z_scores) > max_zscore, "company_id"])%>%pull()
  
  # Filter out rows where the absolute z-score is greater than 3
  df_filtered <- df %>% filter(!(company_id %in% outlier_companies))
...
common_rows <- analysis_data_full_global_no_outlier_per_run %>%
  group_by(company_id,
           ald_sector,
           ald_business_unit) %>%
  mutate(count = n_distinct(run_id)) %>%
  ungroup() %>%
...
analysis_data_full_global_no_outlier <- analysis_data_full_global_no_outlier_per_run %>%
  inner_join(common_rows, by = c("company_id", 
                                 "ald_sector", 
                                 "ald_business_unit"))
...
# for (run_id in c("5776a4b8-476f-48ba-91ae-b83a178d4331", "24e5ddb1-d63b-416f-bc4f-6d5f2d0e0a5f", 
#                  "663fa2de-e808-457b-b234-dca8e736b8a0", "eb594eb8-13a7-461e-b8fb-7cad52a7167b")){
# analysis_data_1_run <- analysis_data_full_global_no_outlier |> dplyr::filter(run_id == run_id)
# 
# agg_analysis_data <- analysis_data_1_run |>
#   # dplyr::filter(.data$net_present_value_difference != 0) |>
#   dplyr::select(.data$company_name, .data$crispy_perc_value_change, .data$pd_difference) |>
...
abcd_data_no_outliers <- abcd_full_global %>% inner_join(
  analysis_data_full_global_no_outlier %>% distinct(company_id, ald_sector, ald_business_unit) %>% mutate(company_id=as.numeric(company_id)),
  by=c("company_id"="company_id",
       "ald_sector"="ald_sector",
       "ald_business_unit"="ald_business_unit")
)
```
...
for (ratio_value in names(sampled_rows)){
  analysis_data_sampled <- analysis_data_full_global_no_outlier %>%
    ...
    group_by(ald_sector, run_id, shock_scenario, shock_year) %>%
    dplyr::summarise(
...
ggplot(
  ...
  geom_line() +
  facet_wrap(value_type ~ ald_sector, scales = "free_y", nrow=2) +
  r2dii.plot::theme_2dii() +
...
for (run_id in c("5776a4b8-476f-48ba-91ae-b83a178d4331", "24e5ddb1-d63b-416f-bc4f-6d5f2d0e0a5f", 
                 "663fa2de-e808-457b-b234-dca8e736b8a0", "eb594eb8-13a7-461e-b8fb-7cad52a7167b")){
analysis_data_1_run <- analysis_data |> dplyr::filter(run_id == run_id)
...
for (run_id in c(
  ...
  plot_data <- data_summary %>% filter(run_id == run_id)
  ...
  p1 <-
    ...
    ggtitle(paste(plot_data[1, "shock_scenario"] %>% pull(),
                  " - shock year :",
...
company_data <- analysis_data%>%filter(company_id==a_company_id)
...
for (run_id in c(
  ...
  plot_data <- company_data %>% filter(run_id == run_id)  %>%
    select(
      shock_scenario,
      shock_year,
      company_name,
  ...
  a_shock_scenario <-  unique(plot_data$shock_scenario)
  a_company_name <-  unique(plot_data$company_name)
  a_shock_year <-  unique(plot_data$shock_year)
  
  
# Reshape the data for NPV
npv_data <- plot_data %>%
    pivot_longer(cols = starts_with("net_present_value"), 
                 names_to = "type", 
                 values_to = "value") %>%
    mutate(type = ifelse(type == "net_present_value_baseline", "Baseline", "Shock"))

# Reshape the data for PD
pd_data <- plot_data %>%
    pivot_longer(cols = starts_with("pd"), 
                 names_to = "type", 
                 values_to = "value") %>%
    mutate(type = ifelse(type == "pd_baseline", "Baseline", "Shock"))

# Plot for NPV
npv_plot <- ggplot(npv_data, aes(x = ald_business_unit, y = value, color=type)) +
...
# Plot for PD
pd_plot <- ggplot(pd_data, aes(x = ald_business_unit, y = value, color=type)) +
...
group_variables_vec <- c("company_name", "company_id","ald_sector", "shock_year", "baseline_scenario", "shock_scenario", "discount_rate")
...
data_cdi_el_plot <- analysis_data |>
  mutate( # THIS MUTATE ONLY IF NO PORTFOLIO PROVIDED
    ...
    exposure_at_default = .data$net_present_value_baseline * .data$loss_given_default,
...
for (run_id_name in  c(
  ...
  data_cdi_el_plot_filtered <- data_cdi_el_plot |> 
    filter(run_id == run_id_name, company_id == company_id)
  
  
  a_shock_scenario <-  unique(data_cdi_el_plot_filtered$shock_scenario)
  a_company_name <-  unique(data_cdi_el_plot_filtered$company_name)
  a_shock_year <-  unique(data_cdi_el_plot_filtered$shock_year)
...
data_cdi_pd_plot <- analysis_data |>
  dplyr::group_by(run_id, shock_scenario, shock_year) |>
  dplyr::group_modify(~ prepare_for_cdi_pd_plots(
    ...
    metrics = c("pd_baseline", "pd_shock", "pd_difference"),
  )) |>
  dplyr::ungroup()

for (shock_scenario in unique(data_cdi_pd_plot$shock_scenario)){
  
  density_plot <- make_density_plots(
    data_cdi_pd_plot %>% filter(shock_scenario == shock_scenario),
                    numeric_values = "pd_difference",
                       density_var = "shock_year",
                       group_variable = "ald_business_unit") + ggtitle(
...
#   dplyr::ungroup()
# 
# for (shock_scenario in unique(data_cdi_npv_plot$shock_scenario)){
# 
#   density_plot <- make_density_plots(
#     data_cdi_npv_plot %>% filter(shock_scenario == shock_scenario),
#     numeric_values = "net_present_value_difference",
#                        density_var = "shock_year",
#                        group_variable = "ald_business_unit") +
...
# 
# top_var_companies <-
#   company_var_accross_scenarios[1:4, ]$company_id
# 
...
#   #   )
#   # ) %>%
#   filter(company_id %in% top_var_companies) %>%
#   select(
#     shock_scenario,
#     shock_year,
#     company_id,
...
# a_shock_year <- 2032
# 
# for (a_company_name in unique(top_plot_companies$company_name)){
# plot_data <- top_plot_companies %>% 
#   filter(company_name == a_company_name,
#          shock_scenario %in% c(
...
#   ggplot(
#     data = plot_data,
#     aes(x = ald_business_unit, 
#         y = values,
#         group = shock_scenario,
...
count_company_bu <-   analysis_data %>%
  distinct(ald_business_unit, company_id) %>%
  group_by(company_id) %>%
  mutate(n_bu = n()) %>%
  ungroup()

company_bu_var_accross_sy <- analysis_data %>%
  filter(shock_scenario == a_shock_scenario,
         shock_year==2032) %>%
  group_by(
    company_id
    # ald_sector,
...
# top_var_companies <-company_var_accross_sy$company_id

top_var_companies <-
  company_bu_var_accross_sy %>% distinct(company_id) %>% sample_n(size = 10) %>% pull()


top_plot_companies <- analysis_data %>%
  # filter(company_id %in% top_var_companies) %>%
  filter(company_id %in% c(2583435, 1990)) %>%
  select(
    shock_scenario,
    shock_year,
    company_id,
...
create_plot <- function(data_subset, strip_color) {
  ggplot(data_subset, aes(x = ald_business_unit, y = values, fill = factor(shock_year))) +
...
data_cdi_el_plot <- analysis_data |>
  ...
  data_cdi_el_plot_filtered <- data_cdi_el_plot |> 
    filter(run_id %in% runs_selection, company_id == company_id)
