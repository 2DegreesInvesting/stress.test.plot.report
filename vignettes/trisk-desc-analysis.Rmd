...
#   group_by(portfolio.company_id) %>%
#   summarise(
#     npv_change_var = var(npv_change_var, na.rm=T),
#     pd_diff_var = var(pd_diff_var, na.rm=T) ,
#     .groups = "drop"
#   ) %>%
#   arrange(desc(npv_change_var))
...
create_plot <- function(data_subset, strip_color) {
  ggplot(data_subset, aes(x = portfolio.ald_business_unit, y = values, fill = factor(crispy.shock_year))) +
    ...
    theme(
      ...
      axis.title.x = element_blank(),
      axis.title.y = element_blank(),
      strip.text = element_text(color = strip_color) # Set the strip text color
    ) +
    scale_y_continuous(labels = scales::label_percent(accuracy = 1)) +
    coord_flip() +
    facet_wrap(~value_name, scales = "free_x")
...
p1 <- create_plot(data1, "#5D9324") # Color for "NPV Rate of Change"
