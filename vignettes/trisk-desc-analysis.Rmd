...
abcd_input_path <- here::here("data-raw", "mock_st_inputs",
    "abcd_stress_test_input.csv"
  )
companies_activites_rawdata_path <- here::here(
      "data-raw",
      "mock_rawdata",
      "company_activities.csv"
    )
...
abcd_full_global <-
  readr::read_csv(abcd_input_path) %>%
  filter(
...
raw_asset_impact <-readr::read_csv(companies_activites_rawdata_path)

pick_run_ids <-  sample(unique(analysis_data_full_global$run_id), 2)
...
```{r}
for (run_id in pick_run_ids) {
  analysis_data_1_run <- analysis_data_full_global |>
    dplyr::filter(.data$run_id == .env$run_id) |>
    dplyr::inner_join(analysis_data_full_global %>% distinct(company_id, ald_sector, ald_business_unit))

  data_plot <- analysis_data_1_run |>
    dplyr::group_by(term, ald_sector, ald_business_unit) |>
    dplyr::summarise(
      avg_pd_baseline = mean(pd_baseline),
...
for (run_id in pick_run_ids) {
  analysis_data_1_run <- analysis_data_full_global_term_5 |> dplyr::filter(.data$run_id == .env$run_id)
...
```{r}
sampled_rows <- list()
for (uniform_ratio in c(0.1, 0.5, 0.9)) {
  cat(uniform_ratio)

  # Group by ald_sector and ald_business_unit, then apply sampling
  abcd_toydata <- sample_companies_greedy(abcd_data_no_outliers, uniform_ratio, n_sampling_attempts=1)
...
### Countries repartition // companies can be duplicated in a region if they exist in multiple countries in this region

```{r}
raw_asset_impact %>%
  distinct(company_id, ald_location) %>%
  inner_join(abcd_data %>% distinct(company_id), by = c("company_id")) %>%
  group_by(ald_location) %>%
  summarise(n_companies = n(), .groups = "drop") %>%
  arrange(desc(n_companies))
```

The analysis data is then filtered to this subset of abcd to reflect the same outputs in analysis
```{r}
analysis_data <-
...
```{r, fig.height=6}
a_company_id <- sample(unique(analysis_data$company_id), 1)
...
for (run_id in pick_run_ids) {
  ...
  npv_data <- plot_data %>%
    tidyr::pivot_longer(
      cols = starts_with("net_present_value"),
      names_to = "type",
      values_to = "value"
    ) %>%
...
pick_company_ids <- sample(unique(analysis_data$company_id), 3)
...
for (run_id in pick_run_ids) {
  for (company_id in pick_company_ids) {
    plist <- list()
    plot_i <- 1

    data_cdi_el_plot_filtered <- data_cdi_el_plot |>
      dplyr::filter(.data$run_id == .env$run_id, .data$company_id == .env$company_id)
...
a_shock_scenario <- sample(unique(analysis_data$shock_scenario), 1)
...
company_bu_var_accross_sy <- analysis_data %>%
  filter(
    shock_scenario == a_shock_scenario
  ) %>%
  group_by(
    company_id
    # ald_sector,
...
for (a_company_name in unique(top_plot_companies$company_name)) {
  plot_data <- top_plot_companies %>%
    ...
    rename(
      `NPV Rate of Change` = crispy_perc_value_change,
      `PD Difference` = pd_difference
    ) %>%
    tidyr::pivot_longer(
      cols = c(`NPV Rate of Change`, `PD Difference`),
      names_to = "value_name",
      values_to = "values"
    )
...
for (company_id in c(2583435, 1990)) {
  ...
  data_cdi_el_plot_filtered <- data_cdi_el_plot |>
    dplyr::filter(.data$run_id %in% pick_run_ids, company_id == company_id)
