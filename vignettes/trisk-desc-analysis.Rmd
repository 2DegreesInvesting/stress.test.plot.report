...
# sample analysis data
  ...
  volume_prod_full <- sum_total_prod_volume(abcd_full_global)
...
```{r}
analysis_data <- analysis_data_full %>%
  inner_join(abcd_toydata %>% distinct(company_id, ald_sector, ald_business_unit) %>% mutate(company_id=as.character(company_id)),
...
```{r}
analysis_data_1_run <- analysis_data |> dplyr::filter(crispy.run_id == "24e5ddb1-d63b-416f-bc4f-6d5f2d0e0a5f")
...
# Impact of sampling on crispy results
...
for (i in 1:n_samples) {
  companies_sample <-  analysis_data_1_run %>%
    distinct(portfolio.company_id) %>%
    sample_frac(0.1) %>%
    pull()
  
  all_samples[[i]] <- companies_sample
}

crispy_samples <- list()
for (i in 1:length(all_samples)) {
  companies_sample <- all_samples[[i]]
  crispy_indicators <- analysis_data_1_run %>%
    filter(portfolio.company_id %in% companies_sample) %>%
    group_by(portfolio.ald_sector, portfolio.ald_business_unit) %>%
    summarise(
      npv_change = mean(
        (
          crispy.net_present_value_shock - crispy.net_present_value_baseline
        ) / crispy.net_present_value_baseline
        ,
        na.rm = T
      ),
      pd_diff = mean(
        crispy.pd_shock - crispy.pd_baseline
        ,
        na.rm = T
      ),
      .groups="drop"
    )
  crispy_samples[[i]] <- crispy_indicators
}



# Combine all dataframes into one
combined_data <- bind_rows(crispy_samples)

# Calculate means and confidence intervals
data_summary <- combined_data %>%
  group_by(portfolio.ald_business_unit, portfolio.ald_sector) %>%
  summarise(
    mean_npv = mean(npv_change),
    mean_pd = mean(pd_diff),
    sd_npv = sd(npv_change),
    sd_pd = sd(pd_diff),
    count = n(),
    .groups = 'drop'
  ) %>%
  mutate(
    ...
    se_pd = sd_pd / sqrt(count),
    ci_npv = qt(0.975, df = count - 1) * se_npv,
    ci_pd = qt(0.975, df = count - 1) * se_pd
  )

# Reshape data to long format for plotting
data_long <- data_summary %>%
  pivot_longer(
    cols = c(mean_npv, mean_pd),
    names_to = "value_type",
    values_to = "mean_value"
  ) %>%
  mutate(
    ci = case_when(
      value_type == "mean_npv" ~ ci_npv,
      value_type == "mean_pd" ~ ci_pd
    )
  )


# Faceted Bar Plot with different colors for value types
ggplot(data_long, aes(x = portfolio.ald_business_unit, y = mean_value, fill = value_type)) +
  geom_bar(stat = "identity", position = position_dodge()) +
  geom_errorbar(aes(ymin = mean_value - ci, ymax = mean_value + ci), width = 0.2, position = position_dodge()) +
  facet_wrap(value_type~., scales="free_y") +
  scale_fill_manual(values = c("mean_npv" = "blue", "mean_pd" = "red")) +
  labs(y = "Value", x = "Business Unit", fill = "Variable Type") +
  theme_minimal()
...
company_crispy_variance <- analysis_data_full %>%
  ...
  inner_join(
    the_constant_prods %>%
      ...
      group_by(company_id, ald_sector) %>%
      summarise(all_equals=all(all_equals)),
    by = c(
...
topvar_companies_sample <- company_crispy_variance %>%
  group_by(portfolio.company_id) %>% 
  mutate(n_bu=n()) %>% 
  ungroup()%>% 
  ...
  top_n(100) %>% 
  sample_n(6) %>% 
  distinct(portfolio.company_id) %>% pull()


data_plot <- analysis_data_full %>%
  filter(portfolio.company_id %in% topvar_companies_sample) %>%
  

ggplot(data = data_plot, aes(x = portfolio.ald_business_unit, y = crispy_perc_value_change, fill = portfolio.ald_business_unit)) +
    geom_col() +
    coord_flip() +
    theme_minimal() +
    theme(legend.position = "none",
