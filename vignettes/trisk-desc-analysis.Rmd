...
for (run_id in c(
  ...
  data_plot <- analysis_data_1_run |>
    ...
    dplyr::summarise(
      avg_pd_baseline = mean(crispy.pd_baseline),
  ...
  le_plot <- ggplot(data_plot, aes(x = portfolio.term, y = `Average PD`, group = `PD type`, color = `PD type`)) +
    ...
    ggtitle(paste(
      analysis_data_1_run[1, "crispy.shock_scenario"] %>% pull(),
      " - shock year :",
...
for (run_id in c(
  ...
  p1 <-
    ...
    ggtitle(paste(
      plot_data[1, "crispy.shock_scenario"] %>% pull(),
      " - shock year :",
...
for (run_id in c(
  ...
  npv_data <- plot_data %>%
    pivot_longer(
      cols = starts_with("crispy.net_present_value"),
      names_to = "type",
      values_to = "value"
    ) %>%
...
company_bu_var_accross_sy <- analysis_data %>%
  filter(
    crispy.shock_scenario == a_shock_scenario,
