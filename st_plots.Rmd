...
data_el <- data_expected_loss %>%
  ...
  summarise(
    ...
    negative_change = if_else(value_change < 0, TRUE, FALSE),
    expected_loss_after = if_else(negative_change, expected_loss_shock, expected_loss_baseline),
    value_change = abs(value_change)
  ) %>%
  select(-expected_loss_shock) %>%
  pivot_longer(
    cols = c(expected_loss_baseline, value_change, expected_loss_after)
  ) %>%
  mutate(
    state = if_else(name == "expected_loss_baseline", "before", "after"),
    name = if_else(name == "expected_loss_after", "expected_loss_baseline", name)
  ) %>%
  mutate(
    state = factor(state, levels = c("before", "after")),
    name = factor(name, levels = c("value_change", "expected_loss_baseline"))
  )
```

Value change data preparation
...
```{r}
ggplot(data_el, aes(x = state, y = value, fill = name)) +
  geom_bar(stat = "identity") +
  geom_text(data = data_el %>% filter(state == "after"), aes(x = state, y = value, label = value)) +
  scale_y_continuous(expand = expansion(mult = c(0, .1)), labels = comma) +
