...
compute_npv_rate_of_change <- function(crispy_output) {
  npv_rate_of_change_df <- crispy_output |>
    ...
    dplyr::mutate(npv_roc = .data$net_present_value_baseline / .data$net_present_value_shock) |>
    ...
    dplyr::ungroup() |>
    dplyr::mutate(
      metric_name = paste("avg_npv_roc", ald_sector, ald_business_unit, sep = "_"),
...
compute_pd_rate_of_change <- function(crispy_output) {
  pd_rate_of_change_df <- crispy_output |>
    ...
    dplyr::mutate(pd_roc = .data$pd_baseline / .data$pd_shock) |>
    dplyr::group_by(ald_sector, ald_business_unit) |>
    dplyr::summarise_at(dplyr::vars(pd_roc), list(avg_pd_roc = purrr::partial(mean, na.rm = TRUE))) |>
    dplyr::ungroup() |>
    dplyr::mutate(
      metric_name = paste("avg_pd_roc", ald_sector, ald_business_unit, sep = "_"),
...
compute_pd_diff <- function(crispy_output) {
  ...
  metrics_df <- avg_pd_difference |>
    dplyr::mutate(
      metric_name = paste("avg_pd_diff", ald_sector, ald_business_unit, sep = "_"),
...
compute_npv_diff <- function(crispy_ouput) {
  npv_metrics <- crispy_ouput |>
    ...
    dplyr::ungroup() |>
    dplyr::mutate(
      metric_name = paste("avg_npv_diff", ald_sector, ald_business_unit, sep = "_"),
