...
make_mean_pd_diff_sy_plots <- function(Scenarios_Sy, bond_data, color_gradient){
  ...
  for (scenario_name in unique(Scenarios_Sy$scenario_duo)) {
    ...
    # Merge data
    Shock_Year_bond_long <- dplyr::inner_join(bond_data, scenario_data, by=c("term", "company_id"), relationship="many-to-many")

    # Calculate mean pd difference
    mean_shockyear <- Shock_Year_bond_long %>%
      group_by(sector, shock_year) %>% # TODO group by asset_type ?
      summarise(mean_pd_difference = mean(pd_difference), .groups="drop")
    ...
    plotsy <-
      ...
      theme_2dii() +
      labs(x = "shock_year", y = "Mean pd_difference", title = paste("Mean PD Difference by Shock Year \n", scenario_name_for_title))
